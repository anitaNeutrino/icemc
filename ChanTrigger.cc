  ...
  for (int iband=0;iband<5;iband++) { // loop over bands
    ...
    for (int jpt=0; jpt<panel1->GetNvalidPoints(); jpt++){
	  ...
	  //Copy frequency amplitude to screen point
          // Hack alert!: make an "if-else" selecton here to choose between CR and neutrinos:
	  // tmp_vhz[0][k]=tmp_vhz[1][k]=panel1->GetVmmhz_freq(jpt*Anita::NFREQ + k)/sqrt(2)/(anita1->TIMESTEP*1.E6);
	  tmp_vhz[0][k]=tmp_vhz[1][k]=panel1->GetVmmhz_freq(jpt*Anita::NFREQ + k);
	  // cout << tmp_vhz[0][k] << endl;
          ...
          // Hack alert!
	  // bvv: temporarily disabled for debugging: anita1->AntennaGain(settings1, hitangle_e, hitangle_h, e_component, h_component, k, tmp_vhz[0][k], tmp_vhz[1][k]);
