...
namespace icemc {
  ...
  public:
    ...
    void applyRoughness(const Settings& settings1, const int& inu, Interaction* interaction1,  RayTracer* ray1, Screen* panel1, Antarctica* antarctica1, Balloon* bn1, const AskaryanRadiationModel* askFreqGen, Anita* anita1, const Shower& showerProps);
    ...
    int GetDirection(const Settings *settings1,  Interaction *interaction1,  const TVector3 &refr,  double deltheta_em,  double deltheta_had,  const Shower& sp,  double vmmhz1m_max,  double r_fromballoon,  RayTracer *ray1,  const AskaryanRadiationModel* askFreqGen,  Geoid::Position posnu,  Anita *anita1,  Balloon *bn1,  TVector3 &nnu,  double& costhetanu,  double& theta_threshold) ;
    ...
    // // @todo constify... needs some love to constify
    // void Summarize(const Settings *settings1,  Anita* anita1,  Source::Spectra *spectra1, const AskaryanRadiationModel* askFreqGen,  ConnollyEtAl2011 *primary1,  double,  double eventsfound,  double,  double,  double,  double*,  double,  double,  double&,  double&,  double&,  double&, TString);
