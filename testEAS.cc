...
int main(int argc,  char **argv) {
  ...
  for (inu = 0; inu < NNU; inu++) {
	...
	if (!settings1->BORESIGHTS) {
	  bn1->GetEcompHcompkvector(n_eplane,  n_hplane,  n_normal,  direction2bn, e_component_kvector[count_rx],  h_component_kvector[count_rx],  n_component_kvector[count_rx]);
