...
void AntTrigger::WhichBandsPass(Settings *settings1, Anita *anita1, GlobalTrigger *globaltrig1, Balloon *bn1, int ilayer, int ifold, int inu, double dangle, double emfrac, double hadfrac){
	...
	anita1->iminbin[j]=0.; // we start to look for single channel triggers firing
	...
	anita1->imaxbin[j]=anita1->NFOUR/2;
...
// 	  for (int i=0;i<anita1->NFOUR/4;i++) {
// 	    cout << "v_banding is " << v_banding_rfcm_e_forfft[j][i] << "\n";
// 	  }
	...
	for (int j=0;j<5;j++) {
	  if (settings1->TRIGGERSCHEME == 2 || settings1->TRIGGERSCHEME == 3 || settings1->TRIGGERSCHEME == 4 || settings1->TRIGGERSCHEME == 5){
	    for (int k=0;k<anita1->NFOUR/2-(int)(anita1->maxt_diode/anita1->TIMESTEP);k++) {
	      anita1->maxbin_fortotal[j]=anita1->NFOUR/2-(int)(anita1->maxt_diode/anita1->TIMESTEP);
	    ...
	    for (int k=anita1->NFOUR/2-(int)(anita1->maxt_diode/anita1->TIMESTEP);k<anita1->NFOUR/2;k++) {
	      anita1->total_vpol_inanita[j][k]=0.;
	      v_banding_rfcm_e_forfft[j][k]=0.;
	    }
	  }
	  else {
...
int GlobalTrigger::PassesTrigger(Settings *settings1, Anita *anita1, int discones_passing, int mode, int &l3trig, int *l2trig, int *l1trig, int antennaclump, int loctrig[Anita::NLAYERS_MAX][Anita::NPHI_MAX], int *loctrig_nadironly, int inu) {
  ...
  PassesTrigger(settings1,anita1,discones_passing,mode,l3trig,l2trig,l1trig,antennaclump,loctrig,loctrig_nadironly,inu,this_threshold);   

}
  int GlobalTrigger::PassesTrigger(Settings *settings1, Anita *anita1, int discones_passing, int mode, int &l3trig, int *l2trig, int *l1trig, int antennaclump, int loctrig[Anita::NLAYERS_MAX][Anita::NPHI_MAX], int *loctrig_nadironly, int inu,double this_threshold) {
